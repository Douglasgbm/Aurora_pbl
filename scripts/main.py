...
# NA INTEGRIDADE, 1 SIGNIFICA TUDO OK E 0 SIGNIFICA FALHA.
...
modulos_online = input("Módulos online? (S/N): ").upper() == "S"
# O UPPER() CONVERTE A ENTRADA PARA MAIUSCULO, E O == "S" VERIFICA SE O USUARIO
# DIGITOU "S" PARA SIM, RETORNANDO TRUE OU FALSE.

# CONVERTE OS DOIS INDICADORES PARA TEXTO LEGIVEL (USADO NA TELA E NOS REGISTROS).
if integridade == 1:
    integridade_texto = "1 (OK)"
else:
    integridade_texto = "{} (Falha)".format(integridade)
...
relatorio_cenario.append("  Integridade         : {}".format(integridade))
    ...
    for fase in roteiro:
        ...
        for hora_na_fase in range(1, fase["horas"] + 1):
            ...
            if hora_na_fase in fase["manobras"]:
                ...
                texto = "[EVENTO] {}: -{:.1f}%  -> bateria em {:.1f}%".format("Manobra de correcao de rota", CUSTO_MANOBRA, energia)
                relatorio.append(texto)
                if MOSTRAR_VOO_NA_TELA: print(texto)
                houve_evento = True
