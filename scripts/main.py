# =====================================================================
# VERIFICACAO DE DECOLAGEM - PROJETO AURORA
# =====================================================================
# ESTE ARQUIVO FAZ A "FASE 0" DA MISSAO: LE A TELEMETRIA, RODA AS SEIS
# VERIFICACOES DE SEGURANCA, CALCULA A AUTONOMIA E EMITE O PARECER DA
# ANALISE ASSISTIDA POR IA. A LOGICA E A MESMA DE ANTES; O QUE MUDOU E
# A ORGANIZACAO: CADA ETAPA VIROU UMA FUNCAO.
#
# POR QUE FUNCOES? PORQUE A SIMULACAO DE MISSAO (missao.py) PRECISA
# REAPROVEITAR ESTAS ETAPAS SEM COPIAR O CODIGO. UM ARQUIVO SOLTO SO
# PODE SER EXECUTADO; UM ARQUIVO COM FUNCOES PODE SER IMPORTADO.
#
...
# QUALQUER FUNCAO DESTE ARQUIVO PODE LER ESTES NOMES DIRETAMENTE.
...
def ler_telemetria():
    ...
    # OS SEIS VALORES VIAJAM JUNTOS PELO PROGRAMA, ENTAO VAO NUM DICIONARIO:
    # UMA "CAIXA COM ETIQUETAS", ONDE telemetria["energia"] DEVOLVE A ENERGIA.
    telemetria = {
        "temp_interna": temp_interna,
        "temp_externa": temp_externa,
        "integridade": integridade,
        "pressao_tanques": pressao_tanques,
        "energia": energia,
        "modulos_online": modulos_online,
    }
    return telemetria


# =====================================================================
# ETAPA 2 - ANÁLISE ENERGÉTICA (item 1.4)
# =====================================================================
def calcular_energia(energia, capacidade_total):
    ...
    # OS QUATRO RESULTADOS TAMBEM VIAJAM JUNTOS, ENTAO VAO NUM DICIONARIO.
    balanco = {
        "energia_disponivel": energia_disponivel,
        "consumo_real": consumo_real,
        "energia_restante": energia_restante,
        "autonomia_restante": autonomia_restante,
    }
    return balanco
...
def verificar_seguranca(telemetria, balanco):
    # TIRA DOS DICIONARIOS OS VALORES QUE ESTA FUNCAO USA, PARA O CODIGO ABAIXO FICAR LEGIVEL.
    temp_interna = telemetria["temp_interna"]
    temp_externa = telemetria["temp_externa"]
    integridade = telemetria["integridade"]
    pressao_tanques = telemetria["pressao_tanques"]
    energia = telemetria["energia"]
    modulos_online = telemetria["modulos_online"]
    energia_disponivel = balanco["energia_disponivel"]
    consumo_real = balanco["consumo_real"]
    energia_restante = balanco["energia_restante"]
    autonomia_restante = balanco["autonomia_restante"]
    ...
    return decolagem_autorizada


# =====================================================================
# ETAPA 4 - ANÁLISE ASSISTIDA POR IA (item 1.5)
# =====================================================================
...
def analisar_ia(telemetria, balanco):
    temp_interna = telemetria["temp_interna"]
    temp_externa = telemetria["temp_externa"]
    integridade = telemetria["integridade"]
    pressao_tanques = telemetria["pressao_tanques"]
...
# =====================================================================
def emitir_parecer(criticos, alertas, decolagem_autorizada):
    print("")
    print("=" * 62)
    ...
    # A CHAVE PODE TER SIDO DESLIGADA AQUI DENTRO, ENTAO PRECISA SER DEVOLVIDA.
    return decolagem_autorizada


# CLASSIFICAÇÃO AUTOMÁTICA DO CENÁRIO (ÓTIMO / MÉDIO / HORRÍVEL)
def classificar(decolagem_autorizada, alertas):
...
# =====================================================================
# ETAPA 6 - REGISTRO DO CENÁRIO (coleta de dados para o relatório)
# =====================================================================
...
def registrar_cenario(telemetria, balanco, criticos, alertas, decolagem_autorizada):
    temp_interna = telemetria["temp_interna"]
    temp_externa = telemetria["temp_externa"]
    integridade = telemetria["integridade"]
    pressao_tanques = telemetria["pressao_tanques"]
    ...
    classificacao = classificar(decolagem_autorizada, alertas)
...
# =====================================================================
# ESTA FUNCAO "REGE" AS OUTRAS: CHAMA CADA ETAPA E PASSA O RESULTADO DE UMA
# PARA A SEGUINTE. O PARAMETRO registrar TEM VALOR PADRAO True; A SIMULACAO
# DE MISSAO CHAMA COM registrar=False PARA NAO GRAVAR UM CENARIO A CADA VOO.
def executar_verificacao(registrar=True):
    telemetria = ler_telemetria()
    balanco = calcular_energia(telemetria["energia"], CAPACIDADE_TOTAL)
    decolagem_autorizada = verificar_seguranca(telemetria, balanco)
    criticos, alertas = analisar_ia(telemetria, balanco)
    decolagem_autorizada = emitir_parecer(criticos, alertas, decolagem_autorizada)

    if registrar:
        registrar_cenario(telemetria, balanco, criticos, alertas, decolagem_autorizada)

    # DEVOLVE TUDO O QUE QUEM CHAMOU POSSA PRECISAR (A MISSAO USA energia E A CHAVE).
    resultado = {
        "telemetria": telemetria,
        "balanco": balanco,
        "criticos": criticos,
        "alertas": alertas,
        "decolagem_autorizada": decolagem_autorizada,
        "classificacao": classificar(decolagem_autorizada, alertas),
    }
    return resultado


# ESTA LINHA E O QUE PERMITE IMPORTAR O ARQUIVO SEM EXECUTA-LO:
#   - "python scripts/main.py"  -> __name__ VALE "__main__" -> RODA O PROGRAMA.
